...
def criar_cargo():
    dados = request.json
    cargo = CargoService.criar(
        nome=dados.get("nome"),
        descricao=dados.get("descricao"),
        riscos_ids=dados.get("riscos_ids")
    )
...
def gerar_exames_cargo(cargo_id):
    ...
    created = VinculoService.gerar_exames_automaticos_por_cargo(
        cargo_id, tipo_exame_default=tipo, agendar_data=data_agendamento
    )
    return jsonify([
        {"id": e.id, "nome": e.nome, "colaborador_id": e.colaborador_id}
        for e in created
    ])